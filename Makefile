...
build: mod-download
	...
	$(GO) build -ldflags="$(LDFLAGS)" -o $(BIN_DIR)/$(REPOS)-go $(REPOS_SRC)
...
release-amd64: mod-download
	...
	GOOS=darwin GOARCH=amd64 $(GO) build -ldflags="$(LDFLAGS)" -o $(BIN_DIR)/$(REPOS)-darwin-amd64 $(REPOS_SRC)
...
release-arm64: mod-download
	...
	GOOS=darwin GOARCH=arm64 $(GO) build -ldflags="$(LDFLAGS)" -o $(BIN_DIR)/$(REPOS)-darwin-arm64 $(REPOS_SRC)

clean:
	@echo "Cleaning binaries..."
	rm -f $(BIN_DIR)/$(ANALYZE)-* $(BIN_DIR)/$(STATUS)-* $(BIN_DIR)/$(REPOS)-* \
		$(BIN_DIR)/$(ANALYZE)-go $(BIN_DIR)/$(STATUS)-go $(BIN_DIR)/$(REPOS)-go
